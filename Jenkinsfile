  ...
  stages {
    ...
    stage('Docker Build Linux') {
      ...
      steps {
		...
		bat 'powershell Compress-Archive -LiteralPath "C:/output/build-'+env.BUILD_NUMBER+'" -DestinationPath "C:/output/build-'+env.BUILD_NUMBER+'_'+env.BRANCH_NAME+'.zip" -Force'
      }
    }
